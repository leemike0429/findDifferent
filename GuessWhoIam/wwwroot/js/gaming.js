...
Vue.component("user-list", {
    ...
    <div :class="cardStyle(user)" v-for="user in users">
      ...
      <div class="user-info">
        ...
        <div class="user-prop" v-if="user.name == name" >
            ...
            <div id="enemy-bar">
                <img v-for="enemy in enemyList" :src="imageUrl(enemy)" v-on:click="attackSPerson(enemy.id),closeSideBar()">
            </div>
        </div>
    ...
    methods: {
        ...
        attackSPerson(id) {//攻擊別人的三張卡片
            let attacker = this.users.find(x => x.name == this.name).id;
            let attacked = this.users.find(x => x.id == id).id;
        ...
        getMoreTime() {//增加自己秒數的卡片
            if (this.users.find(x => x.name == this.name).time != 0) {
                let attacker = this.users.find(x => x.name == this.name).id;
                let attacked = this.users.find(x => x.name == this.name).id;
    ...
    created() {
        ...
        this.conn.on("Attack", (attacker, card, attacked) => { //[SignalR] 接收來自其他人的卡片效果
            this.users.find(x => x.id == attacker).cards.find(x => x.id == card && x.isUsed == false).isUsed = true;
            switch (card) {
                case 1://阻擋卡id=1
                    if (this.users.find(x => x.name == this.name).id == attacked) {
                        $bus.$emit("block")
                    }
                    break;
                case 2://吸血卡id=2
                    this.users.find(x => x.id == attacked).life = 1;
                    break;
                case 4://搶奪卡id=4 
                    let randomNumArray = []
                    for (let i = 0; i <= 1; i++) {
                        ...
                        var stolencard = this.users.find(x => x.id == attacked).cards[randomNumArray[i]]
                        if (stolencard.isUsed == false) {
                            this.users.find(x => x.id == attacker).cards.push(JSON.parse(JSON.stringify(stolencard)))
                            this.users.find(x => x.id == attacked).cards.find(x => x == stolencard).isUsed = true;
                            return
                        }
                    }
                    break;
...
Vue.component("find-difference", {
    ...
    methods: {
        getImageUrl() {
            ...
            this.conn.on("StartGame", ({ topicIndex, themeId }) => { //[signalR]取得該場遊戲的隨機題目
...
<section class="section_Room">
            ...
            if (roomId != null && roomId != "" && roomId != undefined) {
                this.conn.start()
                    .then(() => {
                        ...
                        this.conn.invoke("Connection", roomId);
                    })
                    .catch((err) => {
                        console.log(err);
                    })
    ...
    created() {
        ...
        this.conn.on("GetLeftList", leftUserList => {
            if (!sessionStorage.getItem("player")) {
                const roomId = sessionStorage.getItem("roomId");
                const playerId = leftUserList.shift();
                this.userName = userList.find(x => x.id == playerId).name;
                sessionStorage.setItem("player", this.userName);
                this.conn.invoke("RecordPlayer", roomId, playerId)
                    .catch((err) => {
                        console.log(err);
                    })
            }
